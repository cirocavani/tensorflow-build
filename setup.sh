...
if [ ! -f build_deps/jdk-8u152-linux-x64.tar.gz ]; then
    curl -k -L \
        -H "Cookie: oraclelicense=accept-securebackup-cookie" \
        http://download.oracle.com/otn-pub/java/jdk/8u152-b16/aa0333dd3019491ca4f6ddbe78cdb6d0/jdk-8u152-linux-x64.tar.gz \
        -o build_deps/jdk-8u152-linux-x64.tar.gz
fi

if [ ! -f build_deps/bazel-0.7.0-installer-linux-x86_64.sh ]; then
    curl -k -L \
        https://github.com/bazelbuild/bazel/releases/download/0.7.0/bazel-0.7.0-installer-linux-x86_64.sh \
        -o build_deps/bazel-0.7.0-installer-linux-x86_64.sh
fi

if [ ! -f build_deps/Miniconda3-4.3.30-Linux-x86_64.sh ]; then
    curl -k -L \
        https://repo.continuum.io/miniconda/Miniconda3-4.3.30-Linux-x86_64.sh \
        -o build_deps/Miniconda3-4.3.30-Linux-x86_64.sh
fi
